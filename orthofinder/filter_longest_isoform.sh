...
grep ">" Apoc.fa | sed 's/>//g' | sort -V > sorted_apoc_ids.txt

#add a zero to the end of every line then keep only the basename, split isoform number (0..n) into its own column
sed 's/$/\.0/' sorted_apoc_ids.txt | sed 's/\.5f\(.*\)//' | sed 's/\(.*\)\./\1 /' > iso_labels.txt

#get the length of each each translatable cds
cat Apoc.fa | awk '$0 ~ ">" {if (NR > 1) {print c;} c=0;printf substr($0,2,100) "\t"; } $0 !~ ">" {c+=length($0);} END { print c; }' | sort -V -k1 | cut -f2 lengths.txt

#paste each together to creater a filter table
paste -d ' ' sorted_apoc_ids.txt iso_labels.txt lengths.txt > table_filter.txt
...
seqkit grep -f longest_iso_ids.txt Apoc.fa > apoc_longest_iso.fa
